...
module.exports = class AddMoneyCommand extends Command {
    constructor (client) {
        super(client, {
            ...
            aliases: ["setcoins"],
            category: "dev",
            description: "Altera a quantidade de PlazaCoins do usuário.",
    ...
    const modouso = new Discord.MessageEmbed() 
        ...
        .setDescription(`**┏━━━━━━━━━━━━━━━━━┓**\n\n  **Comando utilizado de forma errada!**\n\n  **Forma de uso:** \`\`${ras.prefix}setmoney [Usuário] [Quantia]\`\`\n  **Sinônimos:** \`\`${ras.prefix}setcoins [Usuário] [Quantia]\`\`\n\n**┗━━━━━━━━━━━━━━━━━┛**`)
        ...
        if (invalidValue) return message.reply("Não foi inserido um valor ou valor informado é inválido.")
        user.PlazaCoin = Number(value)
        user.save()

        message.reply(`a quantidade de PlazaCoins de **${member.username}** foi alterada para \`${Number(value).toLocaleString()}\` com sucesso!`)
    }
}